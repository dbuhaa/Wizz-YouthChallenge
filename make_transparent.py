...
if __name__ == "__main__":
    ...
    # Process plane
    planes = glob.glob(f"{brain_dir}/plane_green_*.png")
    if planes:
        make_transparent_green(planes[-1], f"{public_dir}/plane.png")
        
    # Process storm
    storms = glob.glob(f"{brain_dir}/storm_green_*.png")
    if storms:
        make_transparent_green(storms[-1], f"{public_dir}/storm.png")
        
    # Process small coin
    small_coins = glob.glob(f"{brain_dir}/coin_small_*.png")
    if small_coins:
        make_transparent_green(small_coins[-1], f"{public_dir}/coin_small.png")
        
    # the other coins will be processed in a follow up run
